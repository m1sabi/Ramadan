...
// ====== CITY AND SOUND DATA ======
const cities = [ "stuttgart", "dortmund", "bremen", "muenchen", "friedrichshafen", "duesseldorf", "essen", "leipzig", "freiburg", "berlin", "nuremberg", "karlsruhe", "mannheim", "frankfurt", "kiel", "hamburg", "koblenz", "ravensburg", "regensburg", "augsburg", "erfurt", "lindau", "konstanz", "tuebingen", "rostock", "flensburg", "sibbhult", "kristianstad", "testcity", "ulm", "neu_ulm" ];
...
// ====== MAIN TABLE FUNCTION ======
  ...
  // ✅ Makka-based Iftar Logic
  if (
    cityLatitude[city] &&
    cityLatitude[city].lat >= 49 &&
    typeof makkahData !== "undefined" &&
    typeof makkahFajr !== "undefined"
  ) {
    ...
    if (makkahDayData && makkahFajrData) {
      ...
      const makkahFastMin = timeToMinutes(makkahDayData.sunset) - timeToMinutes(makkahFajrTime);
...
function checkAndStartCountdown(fajr, maghrib, ishaa) {
  ...
  const times = [timeToMinutes(fajr), timeToMinutes(maghrib), timeToMinutes(ishaa)];
  if (times.some(t => t - currentMin === 2)) startCountdown(120);
}
...
function triggerAzan(prayerName) {
  ...
  if (Notification.permission === "granted") {
    new Notification("📢 الأذان", {
      ...
      icon: image,
      actions: [{ action: "stop", title: "إيقاف الأذان" }]
    });
  }
